...
const addressVersionCheck = (network, address) => {
  try {
    ...
    if (
      _b58check.version === network.pubKeyHash ||
...
const bip39Search = (
  ...
  for (
    let i = Number(accountCountOffset);
    i < Number(accountCountOffset) + Number(_defaultAccountCount);
    i++
  ) {
    for (let j = 0; j < (includeChangeAddresses ? 2 : 1); j++) {
      for (
        let k = Number(addressDepthOffset);
        k < Number(addressDepthOffset) + Number(_defaultAddressDepth);
        k++
      ) {
...
const fromWif = (string, network, versionCheck) => {
  ...
  if (versionCheck) {
    if (
      network.wifAlt &&
      version !== network.wif &&
      network.wifAlt.indexOf(version) === -1
    )
      throw new Error("Invalid network version");
    if (!network.wifAlt && version !== network.wif)
      throw new Error("Invalid network version");
  }
  ...
  if (network.wifAlt) {
    ...
    for (let i = 0; i < network.wifAlt.length; i++) {
      ...
      const _altKP = network.isZcash
        ? new bitcoinZcash.ECPair(d, null, {
            network: _network,
          })
...
const pubkeyToAddress = (pubkey, network) => {
  try {
    ...
    if (network && network.isZcash) {
      address = bitcoinZcash.address.toBase58Check(
        publicKeyHash,
        network.pubKeyHash
      );
    } else if (network && network.isGRS) {
      address = groestlcoinjsLib.address.toBase58Check(
        publicKeyHash,
        network.pubKeyHash
      );
    } else {
...
const xpub = (seed, options) => {
  ...
  const node =
    options && options.network
      ? bip32.fromSeed(_seed, options.network)
      : bip32.fromSeed(_seed);
  let string;
...
const btcToEthPriv = (_wif) => {
  ...
  const ethWallet = new ethersWallet.Wallet(
    ethUtil.bufferToHex(decodedWif.privateKey)
  );
...
const multisigGenerate = (NofN, pubKeys, network) => {
  if (!pubKeys || pubKeys.length < NofN) {
    ...
    const redeemScript =
      network && network.isZcash
        ? bitcoinZcash.script.multisig.output.encode(NofN, _pubKeys)
        : bitcoin.script.multisig.output.encode(NofN, _pubKeys);
    const scriptPubKey =
      network && network.isZcash
        ? bitcoinZcash.script.scriptHash.output.encode(
    ...
    const address = network
      ? network.isZcash
        ? bitcoinZcash.address.fromOutputScript(scriptPubKey, network)
...
const redeemScriptToPubAddress = (scriptPubKey, network) => {
  return network
    ? network.isZcash
      ? bitcoinZcash.address.fromOutputScript(
...
const decodeRedeemScript = (redeemScript, options) => {
  let decodedRedeemScript =
    options && options.network && options.network.isZcash
      ? bitcoinZcash.script.multisig.output.decode(
          Buffer.from(redeemScript, "hex")
        )
      : bitcoin.script.multisig.output.decode(Buffer.from(redeemScript, "hex"));
  ...
  if (options && options.hasOwnProperty("toHex") && options.toHex === true) {
    ...
    for (let i = 0; i < _pubKeys.length; i++) {
      decodedRedeemScript.pubKeys[i] = decodedRedeemScript.pubKeys[i].toString(
        "hex"
      );
    }
  }
...
const pubToElectrumScriptHashHex = (address, network) => {
  ...
  if (network && network.isZcash) {
    ...
    script = bitcoin.address.toOutputScript(
      address,
      network ? network : bitcoinjsNetworks.btc
    );
  }
...
const getAddressVersion = (address) => {
  try {
    ...
    for (let key in bitcoinjsNetworks) {
      if (
        _b58check.version === bitcoinjsNetworks[key].pubKeyHash ||
    ...
    try {
      _b58check = groestlcoinjsLib.address.fromBase58Check(
        address,
        bitcoinjsNetworks.grs
      );
...
const pubToPub = (address, networkSrc, networkDest) => {
  ...
  if (networkSrc && networkSrc.isZcash) {
    ...
    script = bitcoin.address.toOutputScript(
      address,
      networkSrc ? networkSrc : bitcoinjsNetworks.btc
    );
  }

  if (networkDest && networkDest.isZcash) {
    return bitcoinZcash.address.fromOutputScript(
      Buffer.from(script, "hex"),
      networkDest
    );
  } else if (networkDest && networkDest.isGRS) {
    return groestlcoinjsLib.address.fromOutputScript(
      Buffer.from(script, "hex"),
      networkDest
    );
  } else {
    return bitcoin.address.fromOutputScript(
      Buffer.from(script, "hex"),
      networkDest ? networkDest : bitcoinjsNetworks.btc
    );
  }
